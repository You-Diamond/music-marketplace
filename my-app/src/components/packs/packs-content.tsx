...
import { Search, SlidersHorizontal, Layers, Play, ShoppingBag } from "lucide-react"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 xl:grid-cols-5 gap-8 items-start relative">
          ...
          <main className="lg:col-span-3 xl:col-span-4">
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {packs.map((pack) => (
                  <div key={pack.id} className="group bg-zinc-900/20 border border-white/[0.05] p-3 rounded-2xl hover:bg-zinc-900/40 transition-all duration-200 flex flex-col justify-between">
                    <div>
                      <div className="relative aspect-square rounded-xl overflow-hidden mb-3 bg-zinc-900 shadow-inner group/cover">
                        {pack.image ? <img src={pack.image} alt={pack.title} className="object-cover h-full w-full transition-transform duration-300 group-hover/cover:scale-105" /> : <div className="w-full h-full flex items-center justify-center bg-zinc-950 text-zinc-800"><Layers size={32} /></div>}
                      </div>
                      <div className="space-y-0.5 px-0.5">
                        <Link href={`/pack/${pack.id}`}><h3 className="font-semibold text-xs text-white truncate tracking-tight hover:text-purple-400 transition-colors">{pack.title}</h3></Link>
                      </div>
                    </div>
                    <div className="mt-3">
                      ...
                      <Link href={`/pack/${pack.id}`} className="w-full h-7 mt-2 rounded-lg bg-white/[0.03] border border-white/[0.05] hover:bg-white hover:text-black hover:border-transparent text-[10px] font-medium text-white transition-all duration-200 flex items-center justify-center gap-1">
