...
export default function BeatsCatalogPage() {
  ...
  // Инициализация состояний из URL параметров для сохранения состояния при перезагрузке
  ...
  // Интеграция с глобальным плеером
  ...
  // Функция обновления URL и запроса данных треков
  const fetchFilteredBeats = React.useCallback(async () => {
    ...
    try {
      ...
      if (selectedProducer) params.set("producer", selectedProducer) // Передаем в query
      ...
      // Синхронизируем состояние фильтров с URL браузера без перезагрузки страницы
  ...
  // Дебаунс для предотвращения лишних запросов при вводе/скролле ползунков
  React.useEffect(() => {
  ...
  // Синхронизируем изменение query-параметра producer снаружи (например, если перешли по новой ссылке)
  React.useEffect(() => {
    ...
    <div className="min-h-screen bg-[#0c0d12] text-zinc-100 p-4 sm:p-8 lg:p-12 transition-colors">
      <div className="max-w-[1600px] mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          {/* Визуальный плашка-фильтр "Каталог продюсера" */}
          {selectedProducer && (
            <div className="flex items-center gap-2.5 bg-purple-500/10 border border-purple-500/20 px-4 py-2 rounded-xl animate-fade-in">
              ...
              <button 
                ...
                title="Показать всех продюсеров"
              >
                <X size={12} />
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 xl:grid-cols-5 gap-8 items-start relative">
          ...
          {/* Фиксированный Сайдбар (Параметры каталога) */}
          <aside className="bg-zinc-900/20 border border-white/[0.05] p-5 rounded-2xl space-y-6 lg:sticky lg:top-24 max-h-[calc(100vh-7rem)] overflow-y-auto scrollbar-none backdrop-blur-md z-10">
            ...
            {/* Кнопка сброса всех фильтров, включая продюсера */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {beats.map((b) => {
                  ...
                  return (
                    <div
                      key={b.id}
                      className={`group bg-zinc-900/20 border p-3 rounded-2xl hover:bg-zinc-900/40 transition-all duration-200 flex flex-col justify-between ${
                        isCurrent ? "border-purple-500/40 shadow-[0_0_30px_rgba(168,85,247,0.05)]" : "border-white/[0.05]"
                      }`}
                    >
                      <div>
                        <div className="relative aspect-square rounded-xl overflow-hidden mb-3 bg-zinc-900 shadow-inner group/cover">
                          {b.image ? (
                            <img src={b.image} alt={b.title} className="object-cover h-full w-full transition-transform duration-300 group-hover/cover:scale-105" />
                          ) : (
                        ...
                        <div className="space-y-0.5 px-0.5">
                          ...
                          <p className="text-[10px] text-zinc-500 font-light truncate">
                            by{" "}
                            <Link href={`/producer/${b.producer.username}`} className="hover:underline text-zinc-400 font-medium transition-colors">
                              @{b.producer.username}
                            </Link>
                          </p>
                        </div>
                      ...
                      <div className="mt-3">
                        ...
                        <div className="flex items-center gap-1.5 mt-2">
                          <button 
                          onClick={() => handleAddToCart(b.id, b.title)}
                          className="flex-1 h-7 rounded-lg bg-white/[0.03] border border-white/[0.05] hover:bg-white hover:text-black hover:border-transparent text-[10px] font-medium text-white transition-all duration-200 flex items-center justify-center gap-1"
                        >
                          ...
                          <button 
                            ...
                            className={`h-7 w-7 rounded-lg border flex items-center justify-center transition-all duration-200 shrink-0 ${
                              isLiked 
                                ? 'bg-red-500/10 border-red-500/20 text-red-500' 
                                : 'bg-white/[0.01] border-white/[0.05] text-zinc-400 hover:text-white hover:border-white/[0.1]'
                            }`}
                          >
